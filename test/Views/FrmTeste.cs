...
namespace Controle.Views
{
    public partial class FrmTeste : Form
    {
        ...
        int idUsuario = UserSession.User.Id;
        ...
        Dictionary<string, string> tagsRH = new Dictionary<string, string> // dicionario com o tag e text, também a foto do botão.
        {
            { "FUNCIONÁRIOS", "Funcionarios" },//TAG + text / FOTO
            { "FUNÇÃO / CARGOS", "Cargos" },//TAG + text / FOTO
            { "FOLHA DE PAGAMENTO", "sanma_logo_color" }, //TAG + text / FOTO
            { "TAXA DE SERVIÇO", "Taxa_Servico" },//TAG + text / FOTO
            { "LANÇAMENTO DE PONTOS", "Pontos" },//TAG + text / FOTO
        };
        ...
        Dictionary<string, string> TagsSenhas = new Dictionary<string, string> // dicionario com o tag e text, também a foto do botão.
        {
            { "SENHAS", "Senhas" },//TAG + text / FOTO
            { "CATEGORIA DE SENHAS", "Categorias_Senhas" },//TAG + text / FOTO
        };
        Dictionary<string, string> TagsCofres = new Dictionary<string, string> // dicionario com o tag e text, também a foto do botão.
        {
        ...
        private void CarregarPermissoes()
        {
            ...
            {
                // todos os acessos permitidos para o administrador.
                acessosPermitidos = new HashSet<string>(tagsPatrimonios.Keys.Concat(tagsRH.Keys)
               .Concat(TagsFichas.Keys).Concat(TagsOrdemDeServico.Keys)
               .Concat(TagsSenhas.Keys).Concat(TagsCofres.Keys)
               .Concat(TagsGov.Keys).Concat(TagsAgenda.Keys)
               .Concat(TagsSistema.Keys).Concat(TagsRelatorios.Keys));
            }
        }
        ...
        private void DestacarMenu(Button btn)
        {
            ...
            {
                btnPatrimonio, btnRH, btnFichas, btnOrdemDeServico, btnSenhas, btnCofres, btnGov, btnAgenda, btnRelatorios, btnSistema
        ...
        private void ConfigurarMenu()
        {
                ...
                List<Dictionary<string, string>> todosTags = new List<Dictionary<string, string>>
            {
                ...
                TagsSistema,
                TagsRelatorios,
                // Adicione aqui outros dicionários conforme necessário
            };
        ...
        {
            DestacarMenu(btnRH); // Destaca o menu lateral
            RemoverBotoesPorNome("btn"); // Remove os botões gerados   
            CriarLayoutBotoes(5, tagsRH, acessosPermitidos); // QTD Botões Criados + TAG dos botões + Permissões
        }
        ...
        {
            DestacarMenu(btnSenhas); // Destaca o menu lateral
            RemoverBotoesPorNome("btn"); // Remove os botões gerados   
            CriarLayoutBotoes(2, TagsSenhas, acessosPermitidos); // QTD Botões Criados + TAG dos botões + Permissões

        }

        private void btnCofres_Click(object sender, EventArgs e)
        {
            DestacarMenu(btnCofres); // Destaca o menu lateral
            RemoverBotoesPorNome("btn"); // Remove os botões gerados   
            CriarLayoutBotoes(1, TagsCofres, acessosPermitidos); // QTD Botões Criados + TAG dos botões + Permissões
        }
        ...
        private void btnGov_Click(object sender, EventArgs e)
        {
            ...
            RemoverBotoesPorNome("btn"); // Remove os botões gerados   
            CriarLayoutBotoes(1, TagsGov, acessosPermitidos); // QTD Botões Criados + TAG dos botões + Permissões
        }
        ...
        private void btnRelatorios_Click(object sender, EventArgs e)
        {
            ...
            RemoverBotoesPorNome("btn"); // Remove os botões gerados   
            CriarLayoutBotoes(5, TagsRelatorios, acessosPermitidos); // QTD Botões Criados + TAG dos botões + Permissões
        }
        ...
        private void btnSistema_Click(object sender, EventArgs e)
        {
            ...
            RemoverBotoesPorNome("btn"); // Remove os botões gerados   
            CriarLayoutBotoes(5, TagsSistema, acessosPermitidos); // QTD Botões Criados + TAG dos botões + Permissões
        }
